...
import com.uknown.kelompok8_project_akhir_8.homepage.orderpage.ChooseBusActivity;
import com.uknown.kelompok8_project_akhir_8.homepage.orderpage.SearchCityActivity;
...
public class SearchFragment extends Fragment {
    ...
    private void intentBookWithData() {
        if (binding.date.getText().toString().equals(getString(R.string.select_date))){
            Toast.makeText(getActivity().getApplicationContext(), "Please select a departure date", Toast.LENGTH_SHORT).show();
            return;
        } else if (binding.arrivalLocation.getText().equals(getString(R.string.try_pekanbaru)) || getActivity().getSharedPreferences("departune", Context.MODE_PRIVATE).getString("slDepartuneDaerah", null).equals(getString(R.string.try_pekanbaru))){
            Toast.makeText(getActivity().getApplicationContext(), "Please set departune  location", Toast.LENGTH_SHORT).show();
            return;
        } else if (binding.departuneLocation.getText().equals(getString(R.string.try_pekanbaru)) || getActivity().getSharedPreferences("departune", Context.MODE_PRIVATE).getString("slArrivalDaerah", null).equals(getString(R.string.try_pekanbaru))){
            Toast.makeText(getActivity().getApplicationContext(), "Please set arrival  location", Toast.LENGTH_SHORT).show();
            return;
        }
...
//        if(passanger > 10){
//            int getPassanger = Integer.parseInt(String.valueOf(binding.passanger.getText().subSequence(0,1)));
//        } else {
